...
TokenHandler.prototype.getClient = function(request, response) {
  ...
  return Promise.try(this.model.getClient, [{ clientId: credentials.clientId, clientSecret: credentials.clientSecret }, { request }], this.model)
    .then(function(client) {
      if (!client) {
...
TokenHandler.prototype.handleGrantType = function(request, client) {
  ...
  var fns = [
    this.getAccessTokenLifetime(request, client, grantType),
    this.getRefreshTokenLifetime(request, client, grantType)
  ];

  return Promise.all(fns)
    .bind(this)
    .spread(function(accessTokenLifetime, refreshTokenLifetime) {
...
TokenHandler.prototype.getAccessTokenLifetime = function(request, client, grantType) {
  if (!this.model.getAccessTokenLifetime) {
    return this.accessTokenLifetime;
  }

  return Promise.try(this.model.getAccessTokenLifetime, [{ client, grantType }, { request }], this.model)
    .then(function(lifetime) {
      if (lifetime === null || lifetime > 0) {
        return lifetime;
      }

      throw new InvalidArgumentError('Invalid argument: `accessTokenLifetime` must be greater than zero');
    });
};
...
TokenHandler.prototype.getRefreshTokenLifetime = function(request, client, grantType) {
  if (!this.model.getRefreshTokenLifetime) {
    return this.refreshTokenLifetime;
  }

  return Promise.try(this.model.getRefreshTokenLifetime, [{ client, grantType }, { request }], this.model)
    .then(function(lifetime) {
      if (lifetime === null || lifetime > 0) {
        return lifetime;
      }

      throw new InvalidArgumentError('Invalid argument: `refreshTokenLifetime` must be greater than zero');
    });
};
