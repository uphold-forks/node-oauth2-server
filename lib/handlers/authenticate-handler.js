...
AuthenticateHandler.prototype.handle = function(request, response) {
  ...
  return Promise.bind(this)
    ...
    .then(function(token) {
      return this.getAccessToken(request, token);
    })
    .tap(function(token) {
    ...
    .tap(function(token) {
      ...
      return this.validateScope(request, token);
    })
    .tap(function(token) {
...
AuthenticateHandler.prototype.getAccessToken = function(request, token) {
  return Promise.try(this.model.getAccessToken, [{ accessToken: token }, { request }], this.model)
...
AuthenticateHandler.prototype.validateScope = function(request, accessToken) {
  return Promise.try(this.model.validateScope, [{ scope: this.scope, token: accessToken }, { request }], this.model).then(function(scope) {
    if (!scope) {
