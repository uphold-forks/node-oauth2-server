...
AuthorizationCodeGrantType.prototype.handle = function(request, client) {
  ...
  return Promise.bind(this)
    ...
    .tap(function(code) {
      return this.revokeAuthorizationCode(request, code);
    })
    .then(function(code) {
      return this.saveToken(request, code.user, client, code.authorizationCode, scope);
    });
};
...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = function(request, client) {
  ...
  return Promise.try(this.model.getAuthorizationCode, [request.body.code, { request }], this.model)
    .then(function(code) {
      if (!code) {
...
AuthorizationCodeGrantType.prototype.revokeAuthorizationCode = function(request, code) {
  return Promise.try(this.model.revokeAuthorizationCode, [code, { request }], this.model)
    .then(function(code) {
      if (!code) {
        throw new InvalidGrantError('Invalid grant: authorization code is invalid');
      }
...
AuthorizationCodeGrantType.prototype.saveToken = function(request, user, client, authorizationCode, scope) {
  var fns = [
    this.generateAccessToken(request),
    this.generateRefreshToken(request),
  ...
  return Promise.all(fns)
    ...
    .spread(function(accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
      ...
      return this.model.saveToken(token, client, user, { request });
    });
};
