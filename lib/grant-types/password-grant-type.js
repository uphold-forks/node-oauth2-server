...
PasswordGrantType.prototype.getUser = function(request) {
  ...
  return Promise.try(this.model.getUser, [request.body.username, request.body.password, { request }], this.model)
    .then(function(user) {
      if (!user) {
...
PasswordGrantType.prototype.saveToken = function(request, user, client, scope) {
  var fns = [
