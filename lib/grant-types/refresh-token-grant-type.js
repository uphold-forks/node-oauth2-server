...
RefreshTokenGrantType.prototype.getRefreshToken = function(request, client) {
  ...
  return Promise.try(this.model.getRefreshToken, [request.body.refresh_token, { request }], this.model)
    .then(function(token) {
      if (!token) {
...
RefreshTokenGrantType.prototype.revokeToken = function(request, token) {
  return Promise.try(this.model.revokeToken, [token, { request }], this.model)
    .then(function(token) {
      if (!token) {
        throw new InvalidGrantError('Invalid grant: refresh token is invalid');
      }
...
RefreshTokenGrantType.prototype.saveToken = function(request, user, client, scope) {
  var fns = [
