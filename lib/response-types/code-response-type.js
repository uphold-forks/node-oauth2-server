...
CodeResponseType.prototype.handle = function(request, client, user) {
  ...
  return Promise.bind(this)
    .then(function() {
      return this.generateAuthorizationCode(request);
    })
    .then(function(authorizationCode) {
      var scope = this.getScope(request);
      var expiresAt = this.getAuthorizationCodeLifetime();

      return this.saveAuthorizationCode(request, authorizationCode, expiresAt, scope, client, uri, user);
    })
    .tap(function() {
...
CodeResponseType.prototype.generateAuthorizationCode = function(request) {
  if (this.model.generateAuthorizationCode) {
    return Promise.try(this.model.generateAuthorizationCode, { request }, this.model);
  }
...
CodeResponseType.prototype.saveAuthorizationCode = function(request, authorizationCode, expiresAt, scope, client, redirectUri, user) {
  var code = {
  ...
  return Promise.try(this.model.saveAuthorizationCode, [{ client, code, user }, { request }], this.model);
};
